...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
        ...
        make_option('--host', dest='host', default='mail.wservices.ch', help='IP or name of mail server'),
        ...
        make_option('--username', dest='username', default='boletines', help='Username to authenticate with mail server'),
        make_option('--password', dest='password', default='1961Nacho', help='Password to authenticate with mail server'),
        make_option('--ssl', action='store_true', dest='ssl', default=True, help='Use to specify that the connection must be made using SSL'),
    )
    ...
    def create_articles(self, emails):
        ...
        for num, email in emails.iteritems():
            ...
            cuerpo = self.get_email_content(email)
            try:
